...
# 공통 인자 설정
COMMON_ARGS="--domain_name robosuite_stack \
...
echo "=== [1/2] 손목 카메라(wrist) 전용 실험 시작 (stack) ==="
# GPU 자원 경합 및 연산 비결정성 문제 방지를 위해 순차 실행합니다.
python train.py $COMMON_ARGS --work_dir ./data/robosuite_stack/lane_dino_wrist --reward_camera wrist

echo "=== [2/2] 기존 방식(both) 실험 시작 (stack) ==="
python train.py $COMMON_ARGS --work_dir ./data/robosuite_stack/lane_dino_both --reward_camera both

echo "모든 실험이 완료되었습니다. tensorboard --logdir ./data/robosuite_stack 로 결과를 확인하세요."
